...
// node_modules/geometry-extrude/dist/geometry-extrude.js
var require_geometry_extrude = __commonJS({
  "node_modules/geometry-extrude/dist/geometry-extrude.js"(exports, module) {
    (function(global2, factory) {
      typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global2 = typeof globalThis !== "undefined" ? globalThis : global2 || self, factory(global2.geometryExtrude = {}));
      ...
      function isEarHashed(ear, minX, minY, invSize) {
        ...
        while (p && p.z >= minZ && n && n.z <= maxZ) {
          if (p !== ear.prev && p !== ear.next && pointInTriangle(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) && area$1(p.prev, p, p.next) >= 0)
            return false;
          p = p.prevZ;
          if (n !== ear.prev && n !== ear.next && pointInTriangle(a.x, a.y, b.x, b.y, c.x, c.y, n.x, n.y) && area$1(n.prev, n, n.next) >= 0)
            return false;
          n = n.nextZ;
        }
        while (p && p.z >= minZ) {
      ...
      function sortLinked(list) {
        var i2, p, q, e2, tail, numMerges, pSize, qSize, inSize = 1;
        do {
          p = list;
          list = null;
          tail = null;
      ...
      function isValidDiagonal(a, b) {
        ...
        equals(a, b) && area$1(a.prev, a, a.next) > 0 && area$1(b.prev, b, b.next) > 0);
      }
      function area$1(p, q, r2) {
      ...
      function getSqSegDist(p, p1, p2) {
        var x2 = p1[0], y = p1[1], dx = p2[0] - x2, dy = p2[1] - y;
        if (dx !== 0 || dy !== 0) {
          var t2 = ((p[0] - x2) * dx + (p[1] - y) * dy) / (dx * dx + dy * dy);
          if (t2 > 1) {
            x2 = p2[0];
            y = p2[1];
          } else if (t2 > 0) {
      ...
      function normalize(out, v3) {
        ...
        var y = v3[1];
        var z = v3[2];
        var d = Math.sqrt(x2 * x2 + y * y + z * z);
        out[0] = x2 / d;
        out[1] = y / d;
        out[2] = z / d;
        return out;
      }
      function v2Normalize(out, v3) {
        var x2 = v3[0];
        var y = v3[1];
      ...
      function scale(out, v3, s2) {
        out[0] = v3[0] * s2;
        out[1] = v3[1] * s2;
        out[2] = v3[2] * s2;
        return out;
      }
      function scaleAndAdd(out, v12, v22, s2) {
        out[0] = v12[0] + v22[0] * s2;
        out[1] = v12[1] + v22[1] * s2;
        out[2] = v12[2] + v22[2] * s2;
        return out;
      }
      function v2Add(out, v12, v22) {
        out[0] = v12[0] + v22[0];
        out[1] = v12[1] + v22[1];
        return out;
      }
      function v3Sub(out, v12, v22) {
        out[0] = v12[0] - v22[0];
        out[1] = v12[1] - v22[1];
        out[2] = v12[2] - v22[2];
        return out;
      }
      function v3Normalize(out, v3) {
        var x2 = v3[0];
        var y = v3[1];
        var z = v3[2];
        var d = Math.sqrt(x2 * x2 + y * y + z * z);
      ...
      function v3Cross(out, v12, v22) {
        var ax = v12[0], ay = v12[1], az = v12[2], bx = v22[0], by = v22[1], bz = v22[2];
        out[0] = ay * bz - az * by;
        out[1] = az * bx - ax * bz;
        out[2] = ax * by - ay * bx;
        return out;
      ...
      var rel = [];
      function slerp(out, start, end, t2) {
        var cosT = dot(start, end);
        var theta = Math.acos(cosT) * t2;
      ...
      function area(points, start, end) {
        ...
        var area2 = 0;
        for (var i2 = (end - 1) * 2, j = start * 2; j < end * 2; ) {
          var x0 = points[i2];
          var y0 = points[i2 + 1];
          var x1 = points[j];
          var y1 = points[j + 1];
      ...
      function triangulate(vertices2, holes) {
        var dimensions = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 2;
        return earcut$1(vertices2, holes, dimensions);
      }
      ...
      var v2 = [];
      var v = [];
      function innerOffsetPolygon(vertices2, out, start, end, outStart, offset, miterLimit, close, removeIntersections) {
        ...
        for (var i2 = start; i2 < end; i2++) {
          var nextIdx = i2 === end - 1 ? start : i2 + 1;
          var prevIdx = i2 === start ? end - 1 : i2 - 1;
          var x1 = vertices2[prevIdx * 2];
          var y1 = vertices2[prevIdx * 2 + 1];
          var x2 = vertices2[i2 * 2];
          var y2 = vertices2[i2 * 2 + 1];
          var x3 = vertices2[nextIdx * 2];
          var y3 = vertices2[nextIdx * 2 + 1];
          v1[0] = x2 - x1;
          v1[1] = y2 - y1;
          v2[0] = x3 - x2;
          v2[1] = y3 - y2;
          v2Normalize(v1, v1);
          v2Normalize(v2, v2);
          checkMiterLimit && (indicesMap[i2] = cursor);
          ...
          if (!close && i2 === start) {
            ...
            if (needCheckIntersection) {
              if (removeIntersections && prevOffsetX != null) {
                var t2 = lineIntersection(x1, y1, prevOffsetX, prevOffsetY, x2, y2, offsetX, offsetY, tmpIntersection, 0);
                if (t2 >= -0.01 && t2 <= 1 + 0.01) {
      ...
      function offsetPolygon(vertices2, holes, offset, miterLimit, close) {
        var offsetVertices = miterLimit != null ? [] : new Float32Array(vertices2.length);
        var exteriorSize = holes && holes.length ? holes[0] : vertices2.length / 2;
        innerOffsetPolygon(vertices2, offsetVertices, 0, exteriorSize, 0, offset, miterLimit, close, true);
        if (holes) {
          for (var i2 = 0; i2 < holes.length; i2++) {
            var start = holes[i2];
            var end = holes[i2 + 1] || vertices2.length / 2;
            innerOffsetPolygon(vertices2, offsetVertices, start, end, miterLimit != null ? offsetVertices.length / 2 : start, offset, miterLimit, close, false);
          }
        }
        return offsetVertices;
      }
      function reversePoints(points, stride, start, end) {
        for (var i2 = 0; i2 < Math.floor((end - start) / 2); i2++) {
          for (var j = 0; j < stride; j++) {
            var a = (i2 + start) * stride + j;
            var b = (end - i2 - 1) * stride + j;
            var tmp = points[a];
            points[a] = points[b];
            points[b] = tmp;
          }
      ...
      function convertToClockwise(vertices2, holes) {
        var polygonVertexCount = vertices2.length / 2;
        var start = 0;
        var end = holes && holes.length ? holes[0] : polygonVertexCount;
        if (area(vertices2, start, end) > 0) {
          reversePoints(vertices2, 2, start, end);
        }
        for (var h2 = 1; h2 < (holes ? holes.length : 0) + 1; h2++) {
          start = holes[h2 - 1];
          end = holes[h2] || polygonVertexCount;
          if (area(vertices2, start, end) < 0) {
            reversePoints(vertices2, 2, start, end);
          }
        }
      ...
      function generateNormal(indices, position) {
        ...
        for (var i4 = 0; i4 < normals.length; ) {
          v3Set(n, normals[i4], normals[i4 + 1], normals[i4 + 2]);
          v3Normalize(n, n);
          normals[i4++] = n[0];
          normals[i4++] = n[1];
          normals[i4++] = n[2];
      ...
      function addExtrudeSide(out, _ref, start, end, cursors, opts) {
        var vertices2 = _ref.vertices, topVertices = _ref.topVertices, splittedMap = _ref.splittedMap, depth = _ref.depth, rect = _ref.rect;
        ...
        var isDuplicateVertex = splittedMap ? function(idx2) {
          var nextIdx = (idx2 + 1) % ringVertexCount;
          return splittedMap[idx2 + start] === splittedMap[nextIdx + start];
        } : function(idx2) {
          return false;
        };
        if (bevelSize > 0) {
          ...
          for (var k = 0; k < 2; k++) {
            ...
            for (var s2 = 0; s2 <= bevelSegments * splitBevel; s2++) {
              var uLen = 0;
              var prevX = void 0;
              var prevY = void 0;
              for (var i2 = 0; i2 < ringVertexCount; i2++) {
                var idx = (i2 % ringVertexCount + start) * 2;
                var rawIdx = splittedMap ? splittedMap[idx / 2] * 2 : idx;
                _v[0] = vertices2[idx] - topVertices[rawIdx];
                _v[1] = vertices2[idx + 1] - topVertices[rawIdx + 1];
                _v[2] = 0;
                var l = Math.sqrt(_v[0] * _v[0] + _v[1] * _v[1]);
                _v[0] /= l;
                _v[1] /= l;
                var t2 = (Math.floor(s2 / splitBevel) + s2 % splitBevel) / bevelSegments;
                k === 0 ? slerp(_v3, v0, _v, t2) : slerp(_v3, _v, _v2, t2);
                var t22 = k === 0 ? t2 : 1 - t2;
                var a = bevelSize * Math.sin(t22 * Math.PI / 2);
                var b = l * Math.cos(t22 * Math.PI / 2);
                var r2 = bevelSize * l / Math.sqrt(a * a + b * b);
                var x2 = _v3[0] * r2 + topVertices[rawIdx];
                var y = _v3[1] * r2 + topVertices[rawIdx + 1];
                var zz = _v3[2] * r2 + z;
                out.position[cursors.vertex * 3] = x2;
                out.position[cursors.vertex * 3 + 1] = y;
                out.position[cursors.vertex * 3 + 2] = zz;
                if (i2 > 0) {
                  uLen += Math.sqrt((prevX - x2) * (prevX - x2) + (prevY - y) * (prevY - y));
                }
                if (s2 > 0 || k > 0) {
                  ...
                  vLen[i2] += Math.sqrt((prevX2 - x2) * (prevX2 - x2) + (prevY2 - y) * (prevY2 - y) + (prevZ2 - zz) * (prevZ2 - zz));
                }
                out.uv[cursors.vertex * 2] = uLen / size;
                out.uv[cursors.vertex * 2 + 1] = vLen[i2] / size;
                prevX = x2;
                prevY = y;
                cursors.vertex++;
                if (isDuplicateVertex(i2)) {
                  continue;
                }
                if (splitBevel > 1 && s2 % splitBevel || splitBevel === 1 && s2 >= 1) {
                  for (var f3 = 0; f3 < 6; f3++) {
                    var m2 = (quadToTriangle[f3][0] + i2) % ringVertexCount;
                    var n = quadToTriangle[f3][1] + ringCount;
                    out.indices[cursors.index++] = (n - 1) * ringVertexCount + m2 + vertexOffset;
                  }
                }
              }
              ringCount++;
          ...
          for (var _k = 0; _k < 2; _k++) {
            ...
            for (var _i2 = 0; _i2 < ringVertexCount; _i2++) {
              ...
              var _x = vertices2[_idx];
              var _y = vertices2[_idx + 1];
      ...
      function addTopAndBottom(_ref2, out, cursors, opts) {
        var indices = _ref2.indices, topVertices = _ref2.topVertices, rect = _ref2.rect, depth = _ref2.depth;
        if (topVertices.length <= 4) {
          return;
        }
        var vertexOffset = cursors.vertex;
        var indicesLen = indices.length;
        for (var i2 = 0; i2 < indicesLen; i2++) {
          out.indices[cursors.index++] = vertexOffset + indices[i2];
        }
        var size = Math.max(rect.width, rect.height);
        for (var k = 0; k < (opts.excludeBottom ? 1 : 2); k++) {
          for (var _i4 = 0; _i4 < topVertices.length; _i4 += 2) {
            var x2 = topVertices[_i4];
            var y = topVertices[_i4 + 1];
            var vtx3 = cursors.vertex * 3;
            var vtx2 = cursors.vertex * 2;
            out.position[vtx3] = x2;
            out.position[vtx3 + 1] = y;
            out.position[vtx3 + 2] = (1 - k) * depth;
            out.uv[vtx2] = (x2 - rect.x) / size;
      ...
      function splitVertices(vertices2, holes, smoothSide, smoothSideThreshold) {
      ...
      function innerExtrudeTriangulatedPolygon(preparedData, opts) {
        ...
        for (var p = 0; p < preparedData.length; p++) {
          var _preparedData$p = preparedData[p], indices = _preparedData$p.indices, _vertices = _preparedData$p.vertices, splittedMap = _preparedData$p.splittedMap, topVertices = _preparedData$p.topVertices, holes = _preparedData$p.holes, depth = _preparedData$p.depth;
          var bevelSize = Math.min(depth / 2, opts.bevelSize);
          ...
          var start = 0;
          var end = 0;
          for (var h2 = 0; h2 < holes.length + 1; h2++) {
            if (h2 === 0) {
              end = holes.length ? holes[0] : _vertices.length / 2;
            } else {
              start = holes[h2 - 1];
              end = holes[h2] || _vertices.length / 2;
            }
        ...
        var data = {
          ...
          indices: new (vertexCount > 65535 ? Uint32Array : Uint16Array)(indexCount),
        ...
        for (var _d = 0; _d < preparedData.length; _d++) {
          var _preparedData$_d = preparedData[_d], _holes = _preparedData$_d.holes, _vertices2 = _preparedData$_d.vertices;
        ...
        for (var i2 = 0; i2 < data.uv.length; i2++) {
          var val = data.uv[i2];
      ...
      function convertPolylineToTriangulatedPolygon(polyline, polylineIdx, opts) {
        ...
        var scale2 = opts.scale || [1, 1];
        for (var i2 = 0, k = 0; i2 < pointCount; i2++) {
          points[k++] = polyline[i2][0] * scale2[0] + translate[0];
          points[k++] = polyline[i2][1] * scale2[1] + translate[1];
        }
        ...
        var indices = new (polygonVertexCount > 65535 ? Uint32Array : Uint16Array)(((pointCount - 1) * 2 + (polygonVertexCount - pointCount * 2)) * 3);
        var off = 0;
        for (var _i8 = 0; _i8 < pointCount - 1; _i8++) {
          ...
          if (insideIndicesMap[i22] - insideIndicesMap[_i8] === 2) {
            indices[off++] = insideIndicesMap[_i8] + 2 + outsidePointCount;
            indices[off++] = insideIndicesMap[_i8] + 1 + outsidePointCount;
            indices[off++] = outsidePointCount - outsideIndicesMap[i22] - 1;
          } else if (outsideIndicesMap[i22] - outsideIndicesMap[_i8] === 2) {
            indices[off++] = insideIndicesMap[i22] + outsidePointCount;
            indices[off++] = outsidePointCount - 1 - (outsideIndicesMap[_i8] + 1);
        ...
        return {
          ...
          topVertices,
          rect: {
            x: boundingRect.x * scale2[0] + translate[0],
            y: boundingRect.y * scale2[1] + translate[1],
            width: boundingRect.width * scale2[0],
            height: boundingRect.height * scale2[1]
          },
      ...
      function extrudePolygon2(polygons, opts) {
        opts = Object.assign({}, opts);
        var min = [Infinity, Infinity];
        var max = [-Infinity, -Infinity];
        for (var i2 = 0; i2 < polygons.length; i2++) {
          updateBoundingRect(polygons[i2][0], min, max);
        }
        ...
        for (var _i9 = 0; _i9 < polygons.length; _i9++) {
          ...
          var simplifyTolerance = opts.simplify / Math.max(scale2[0], scale2[1]);
          ...
          var _earcut$flatten = earcut$1.flatten(newPolygon), _vertices3 = _earcut$flatten.vertices, holes = _earcut$flatten.holes, dimensions = _earcut$flatten.dimensions;
          for (var k = 0; k < _vertices3.length; ) {
            _vertices3[k] = _vertices3[k++] * scale2[0] + translate[0];
            _vertices3[k] = _vertices3[k++] * scale2[1] + translate[1];
          }
      ...
      function extrudePolyline(polylines, opts) {
        ...
        for (var i2 = 0; i2 < polylines.length; i2++) {
          updateBoundingRect(polylines[i2], min, max);
        }
        opts.boundingRect = opts.boundingRect || {
          x: min[0],
          y: min[1],
      ...
      function updateBoundingRect(points, min, max) {
        for (var i2 = 0; i2 < points.length; i2++) {
          min[0] = Math.min(points[i2][0], min[0]);
          min[1] = Math.min(points[i2][1], min[1]);
          max[0] = Math.max(points[i2][0], max[0]);
          max[1] = Math.max(points[i2][1], max[1]);
        }
      }
      function extrudeGeoJSON(geojson, opts) {
          ...
          if (geometry && geometry.coordinates) {
            switch (geometry.type) {
              case "LineString":
                ...
                polylineFeatureIndices.push(i2);
        ...
        return {
          ...
          polygon: extrudePolygon2(polygons, Object.assign(opts, {
            depth: function depth(idx) {
              if (typeof originalDepth === "function") {
      ...
      exports2.extrudeGeoJSON = extrudeGeoJSON;
      exports2.extrudePolygon = extrudePolygon2;
      exports2.extrudePolyline = extrudePolyline;
      exports2.flatten = flatten;
      exports2.offsetPolygon = offsetPolygon;
      exports2.triangulate = triangulate;
...
// generator_GFApolygonscopy.js
var import_geometry_extrude = __toESM(require_geometry_extrude());
...
function createMesh() {
  const { indices, position } = (0, import_geometry_extrude.extrudePolygon)(
    [[[[0, 0], [10, 0], [10, 10], [0, 0]]]],
    {
      depth: 20
    }
...
async function processRequest(req, res, token2) {
  console.log("Request received");
  if (["https://app.autodeskforma.eu", "https://app.autodeskforma.com"].includes(
    req.headers.origin
  )) {
  ...
  if (requestBody.generatorValues.polygon == null) {
    requestBody.generatorValues.polygon = requestBody.generatorValues["965c705a-4e37-4362-b612-da97e5650303"];
  }
  if (requestBody.generatorValues.height == null) {
    requestBody.generatorValues.height = requestBody.generatorValues["8853b205-7749-40b3-a73a-198c9e50dafe"];
  }
  if (requestBody.generatorValues.polygon == null) {
...
var token = await getApsToken();
var server = createServer((req, res) => processRequest(req, res, token));
server.listen(8080);
